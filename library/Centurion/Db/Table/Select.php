...
class Centurion_Db_Table_Select extends Zend_Db_Table_Select
{
    ...
    public function addOneToOneTable($rule, $localTable = null, $cols = null, $joinType = self::JOIN_TYPE_INNER, $foreignRule = null)
    {
        ...
        $joinCond = sprintf('%s.%s = %s.%s', $this->_adapter->quoteIdentifier($depTableName),
        ...
        do {
            ...
            if (in_array($rule[1], array_keys($manyToManyMap))) {
                ...
                if (null !== $nextRule) {
                    ...
                    $sqlField = sprintf('%s.%s', $this->_adapter->quoteIdentifier($localTable->info(Centurion_Db_Table_Abstract::NAME)),
            ...
            if (null === $sqlField && in_array($nextRule, $foreignTable->info(Centurion_Db_Table_Abstract::COLS))) {
                $sqlField = sprintf('%s.%s', $this->_adapter->quoteIdentifier($foreignTable->info(Centurion_Db_Table_Abstract::NAME)),
    ...
    public function hydrate($dependences, $joinType = self::JOIN_TYPE_LEFT)
    {
        ...
        foreach ($dependences as $dependence) {
            ...
            $refMap = $this->getTable()->getReferenceMap($dependence);
