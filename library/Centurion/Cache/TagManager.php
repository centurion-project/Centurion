    ...
    public static function getTagOf($tag)
    {
        if (is_string($tag)) {
            ...
            if ($tag instanceof Zend_Db_Table_Row_Abstract) {
                $pk = is_string($tag->pk) || is_int($tag->pk) ? $tag->pk : md5(serialize($tag->pk));
                return sprintf('__%s__%s', $tag->getTable()->info(Centurion_Db_Table_Abstract::NAME), $pk);
            } elseif ($tag instanceof Zend_Db_Table_Abstract){
                return sprintf('__%s', $tag->info(Centurion_Db_Table_Abstract::NAME));
            }
        }
    }
