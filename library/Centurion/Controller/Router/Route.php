...
{
    protected static $_translatedMessage = null;
    ...
    public function match($path, $partial = false)
    {
        if ($this->_isTranslated) {
            if (null === self::$_translatedMessage) {
                self::$_translatedMessage = array();

                //TODO: optimise it
                $cache = Zend_Registry::get('Cache_Manager')->getCache('core');
                $id = sprintf('Route_Translated_Message_%s', $this->getTranslator()->getLocale());

                if (!(self::$_translatedMessage = $cache->load($id))) {
                    $translateMessages = $this->getTranslator()->getMessages($this->getTranslator()->getLocale());

                    if (is_array($translateMessages)) {
                        foreach($translateMessages as $key => $val) {
                            self::$_translatedMessage[Centurion_Inflector::slugify($key)][$val] = Centurion_Inflector::slugify($val);
                        }
                    }

                    $cache->save(self::$_translatedMessage, $id, array('__translation_translation'));
                }
            }
        }
        ...
        if ($path !== '') {
            ...
            foreach ($path as $pos => $pathPart) {
                ...
                if ($this->_isTranslated && (substr($part, 0, 1) === '@' && substr($part, 1, 1) !== '@'
                    ...
                    if (isset(self::$_translatedMessage[$part])) {
                        foreach(self::$_translatedMessage[$part] as $translation) {
                            if ($pathPart === $translation) {
                                $pathPart = $part;
                                break;
                            }
                        }
