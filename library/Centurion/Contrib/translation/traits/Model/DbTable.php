...
//    )
    ...
    public function onJoinInner($signal, $sender, $select, $name)
    {
        ...
        $childName = 'child_' . $this->_modelName;
...
//        }
        ...
        foreach ($spec[Translation_Traits_Model_DbTable::TRANSLATED_FIELDS] as $col) {
            array_push($childCols, sprintf($childName . '.%s AS %s%s', $col, $this->_localizedColsPrefix, $col));
...
//            $method = 'joinInner';
        ...
        $select->$method(sprintf('%s AS ' . $childName, $this->_modelInfo[Centurion_Db_Table_Abstract::NAME]),
                         new Zend_Db_Expr(sprintf($childName . '.original_id = %s.id AND ' . $childName . '.language_id = %s', $this->_modelInfo[Centurion_Db_Table_Abstract::NAME], $session->language['id'])),
        ...
        if (!$this->_model->ifNotExistsGetDefault()) {
            $select->where(new Zend_Db_Expr(sprintf($childName . '.language_id = %u OR %s.language_id = %u', $session->language['id'], $this->_modelInfo[Centurion_Db_Table_Abstract::NAME], $session->language['id'])));
        }
    }
