...
class Translation_Traits_Form_Model extends Centurion_Traits_Form_Model_Abstract
{
    ...
    public function onFormGetToolbar()
    {
        ...
        if ($this->hasInstance()) {
            foreach ($langues as $lang) {
                if ($lang->id != $this->_instance->{Translation_Traits_Model_DbTable::LANGUAGE_FIELD}) {
                    if ($lang->id == Translation_Traits_Common::getDefaultLanguage()->id && null !== $this->_instance->{$this->_instance->getTable()->getOriginalRefRule()}) {
                        $tb[] = sprintf('<a href="%s" class="ui-button ui-button-text-only ui-button-bg-white-gradient ui-button-text-red"><span class="ui-button-text">Edit in %s</span></a>', $this->getView()->url(array('action' => 'get',
                                                                                                               'id'     => $this->_instance->{$this->_instance->getTable()->getOriginalRefRule()}->pk,
                                                                                                               'lang'   => null,
                                                                                                               'from'   => null)),
                                                                                   ucfirst($lang->locale));
                    } else {
                        $tb[] = sprintf('<a href="%s" class="ui-button ui-button-text-only ui-button-bg-white-gradient ui-button-text-red"><span class="ui-button-text">Edit in %s</span></a>', $this->getView()->url(array('action'   => 'translate',
                                                                                                               'id'       => null,
                                                                                                               'lang'     => $lang->id,
                                                                                                               'from'     => $this->_instance->id)),
                                                                                   ucfirst($lang->locale));
                    }
                }
            }
        } elseif ($this->getValue(Translation_Traits_Model_DbTable::ORIGINAL_FIELD)) {
            $tb[] = sprintf('<a href="%s" class="ui-button ui-button-text-only ui-button-bg-white-gradient ui-button-text-red"><span class="ui-button-text">Back to %s</span></a>', $this->getView()->url(array('action' => 'get',
                                                                                                   'id'   => $this->getValue(Translation_Traits_Model_DbTable::ORIGINAL_FIELD),
                                                                                                   'from' => null,
                                                                                                   'lang' => null)),
                                                                       Translation_Traits_Common::getDefaultLanguage()->locale);
        }

        $this->_toolbarElems = array_merge($this->_toolbarElems, $tb);
    ...
    protected function _generateInfoField($baseName, $label, $value)
    {
