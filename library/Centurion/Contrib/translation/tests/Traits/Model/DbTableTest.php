...
class Translation_Test_Traits_Model_DbTableTest
    ...
    /**
     * Check if select object build from the model has the trait Translation_Traits_Model_DbTable_Select
     */
    public function testMethodOnSelectBehavior(){
        ...
        //Check if the trait is loaded in this select obejct
        $found = false;
        foreach($traitQueue as $trait){
            if($trait instanceof Translation_Traits_Model_DbTable_Select){
                $found = true;
            }
        }
        ...
        $this->assertTrue(
            $found,
            'Error, the select object must implements the trait Translation_Traits_Model_DbTable_Select, it can be added dynamicaly by the model');
    }
}