...
class Translation_AdminController extends Centurion_Controller_CRUD
{
    ...
    public function filterCaseInsensitive(Centurion_Db_Table_Select $where, $values, $param)
    {
        return $where->where(new Zend_Db_Expr($where->getAdapter()->quoteInto('uid like ? COLLATE utf8_general_ci', $values)));
    }
    ...
    public function init()
    {
        ...
        $this->_filters = array(
                'uid' => array('behavior' => self::FILTER_BEHAVIOR_CONTAINS, 'label' => $this->view->translate('Id')),
                'reference' => array(
